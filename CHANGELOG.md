...
- Coordinated release for MCP 0.2.12's client-neutral, direct-use onboarding.
  The Rust API and wire contract are unchanged from 0.2.11.
...
- Coordinated release for the restart-safe MCP maker continuation contract;
  the Rust quickstart and byte-exact verification APIs are unchanged.

## 0.2.5

- Add `platform_maker_start`, `platform_maker_stop`, and split quickstart
  prepare/submit helpers with human decimal sizing, safe Strand/Current
  defaults, exact transaction verification, idempotent submission, and
  chain-derived confirmation.
